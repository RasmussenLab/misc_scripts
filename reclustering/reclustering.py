...
def read_clusters(filehandle, min_size: int = 1) -> dict[str, set[str]]:
    ...
    if header.rstrip(" \n") != "clustername\tcontigname":
        raise ValueError(
            'Expected cluster TSV file to start with header: "clustername\\tcontigname"'
        )
...
"Contigname -> integer representing genus"
def load_taxonomy(path) -> dict[str, int]:
    result: dict[str, int] = dict()
    genera_to_index: dict[str, int] = dict()
    with open(path) as file:
        header = next(file, None)
        if header is None:
            raise ValueError("Empty taxonomy file - expected header")
        if not header.startswith("contigs\tpredictions"):
            raise ValueError("Expected taxonomy file to start with columns contigs\\tpredictions")
        for line in file:
            fields = line.split('\t')
            contigname = fields[0]
            ranks = fields[1].split(';')
            if len(ranks) > 5:
                genus = ranks[5]
            else:
                genus = "mock"
            index = genera_to_index.setdefault(genus, len(genera_to_index))
            result[contigname] = index
    
    return result

def recluster_bins(
    clusters_path, # path to clusters tsv time
    latents_path, # path to latent
    contigs_path, # path to contigs FASTA
    contignames_all, # list of all contig names
    minfasta, # minimum bin size [to do what?]
    binned_length, # min contig length
    num_process,
    out_dir,
    random_seed,
    algorithm, # 'dbscan' or 'kmeans', exactly 
    hmmout_path=None,
    ...
    if (algorithm == "dbscan") and predictions_path:
        clusters_labels_map = load_taxonomy(predictions_path)
    else:
    ...
    with open(os.path.join(out_dir, "clusters_reclustered.tsv"), "w") as file:
        new_bins = defaultdict(list)
        assert len(contignames_all) == len(contig_labels_reclustered)
        for (header, label) in zip(contignames_all, contig_labels_reclustered):
            new_bins[label].append(header)
        print("clustername\tcontigname", file=file)
        for (label, headers) in new_bins.items():
            for header in headers:
                print(label, header, sep='\t', file=file)
    return None

if __name__ == "__main__":
    parser = argparse.ArgumentParser()

    parser.add_argument("clusters_path", help="path to clusters TSV file")
    parser.add_argument("latent_path", help="path to latent space .npz file")
    parser.add_argument("contigs_path", help="path to FASTA file of contigs")
    parser.add_argument(
        "headers_path", help="path to text file with FASTA headers, in order, without >"
    )
    parser.add_argument("out_dir", help="output directory (must exist)")
    parser.add_argument("algorithm", choices=["dbscan", "kmeans"], help="algorithm")
    parser.add_argument(
        "--num_process", type=int, default=8, help="Number of processed [8]"
    )
    parser.add_argument(
        "--minfasta", type=int, default=200000, help="Minimum bin size [200000]"
    )
    parser.add_argument(
        "--minlength", type=int, default=2000, help="Minimum contig size [2000]"
    )
    parser.add_argument("--seed", type=int, default="314", help="random seed")
    parser.add_argument(
        "--hmmout_path",
        default=None,
        help="Path to HMM markers [predict if not passed]",
    )
    parser.add_argument(
        "--taxpath",
        help="Path to tax predictions if DBSCAN with col name 'predictions' and 7 ranks sep by ;",
    )

    args = parser.parse_args()

    with open(args.headers_path) as file:
        headers = [line.rstrip() for line in file]

    while len(headers) > 0 and len(headers[-1]) == 0:
        headers.pop()
